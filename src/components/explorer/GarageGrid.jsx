...
const CELL = {
  ...
  unknown: 'border-concrete bg-plaster',
}
...
export default function GarageGrid({ title, items = [] }) {
  ...
  return (
    <section className="rounded-2xl border border-concrete bg-white p-4">
      ...
      <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 lg:grid-cols-8">
        {items.map((item) => (
          <button
            ...
            className={`rounded-md border p-2 text-center text-xs font-medium transition ${CELL[statusOf(item.status)]}`}
          >
            {item.number}
          </button>
        ))}
      ...
      {selected && (
        <div className="mt-3 flex flex-wrap items-center gap-4 rounded-lg bg-plaster p-3 text-sm">
          <span className="font-display text-lg text-ink">{selected.number}</span>
